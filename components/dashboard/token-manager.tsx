    ...
    <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
      <Panel>
        <div className="space-y-3">
          <h2 className="text-xl font-semibold text-foreground">Create token</h2>
        ...
        <form action={formAction} className="mt-6 space-y-4">
          <label className="block space-y-2">
            <span className="text-sm font-medium text-foreground/84">Token name</span>
            <Input
              name="name"
          ...
          <label className="block space-y-2">
            <span className="text-sm font-medium text-foreground/84">Scope</span>
      ...
      <Panel>
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-foreground">Active tokens</h2>
            <p className="mt-2 text-sm leading-7 text-foreground/62">
              Review prefixes, scopes, and last-used activity before you rotate.
            </p>
          </div>
          <p className="text-sm text-foreground/46">{tokens.length} total</p>
        </div>
              ...
              {tokens.length ? (
                tokens.map((token) => (
                  <tr key={token.id} className="bg-black/8">
                    ...
                    <td className="px-4 py-4 text-foreground/64">
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-foreground/46">
